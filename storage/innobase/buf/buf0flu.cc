                                     ...
                                     ulint n_flushed, ulint n_to_flush)
{
       ...
       ++id, ++id_fold)
  {
    ...
    if (buf_page_t *bpage= buf_pool.page_hash.get(id, chain))
    {
      ...
      else if ((!evict || bpage->is_old()) &&
               !buf_pool.watch_is_sentinel(*bpage) &&
                                     ...
                                     flush_counters_t *n)
{
        ...
        recv_recovery_is_on()); ++scanned)
  {
    ...
    else if (state < buf_page_t::READ_FIX)
    {
      ...
      /* For the temporary tablespace, LRU flushing will always evict pages. */
      const bool do_evict= oldest_modification == 2 || evict;
...
static void buf_flush_page_cleaner()
{
  ...
  for (;;)
  {
    ...
    const bool LRU_idle= buf_pool.try_LRU_scan;

    if (!oldest_lsn && LRU_idle)
    {
      if (UNIV_UNLIKELY(lsn_limit != 0))
      {
        buf_flush_sync_lsn= 0;
    ...
    if (lsn_limit || soft_lsn_limit || !LRU_idle);
